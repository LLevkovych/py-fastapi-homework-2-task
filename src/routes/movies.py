...
async def list_movies(
    ...
    query = (select(MovieModel).options(
        ...
        selectinload(MovieModel.actors),
        selectinload(MovieModel.languages),
    ).order_by(MovieModel.id.desc()).offset(offset).limit(per_page))
...
async def update_movie(movie_id: int, movie_data: MovieUpdateSchema, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if movie_data.genres:
            movie.genres = await process_many_to_many(db, GenreModel, "name", movie_data.genres)
        if movie_data.actors:
            movie.actors = await process_many_to_many(db, ActorModel, "name", movie_data.actors)
        if movie_data.languages:
            movie.languages = await process_many_to_many(db, LanguageModel, "name", movie_data.languages)
        ...
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Input data is invalid")
